...
public final class Main {
    ...
    public static void main(String[] args) {
        ...
        if (!isZero(new Vector(new Point3D(0, 3, 4)).length() - 5))
        ...
        Vector v = new Vector( new Point3D(1, 2, 3));
        ...
        if (!Point3D.ZERO.equals(p1.add(new Vector(new Point3D(-1, -2, -3)))))
            out.println("ERROR: Point + Vector does not work correctly");
        if (!new Vector( new Point3D(1, 1, 1)).equals(new Point3D(2, 3, 4).subtract(p1)))
